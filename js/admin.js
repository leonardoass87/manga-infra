...
if (window.AdminPanelLoaded) {
    ...
    if (!window.API_BASE_URL) window.API_BASE_URL = window.location.origin + '/api';
    ...
    class AdminPanel {
      ...
      async checkAuth() {
        try {
          if (typeof authManager?.verifyToken === 'function') await authManager.verifyToken();
        } catch (_) {}
        if (!authManager.isLoggedIn() || !authManager.isAdmin()) window.location.href = 'login.html';
      }
  
      bindEvents() {
        document.getElementById('add-manga-btn')?.addEventListener('click', () => this.openModal());
        document.getElementById('modal-close')?.addEventListener('click', () => this.closeModal());
        document.getElementById('cancel-btn')?.addEventListener('click', () => this.closeModal());
        document.getElementById('confirm-close')?.addEventListener('click', () => this.closeConfirmModal());
        document.getElementById('confirm-cancel')?.addEventListener('click', () => this.closeConfirmModal());

        document.getElementById('manga-form')?.addEventListener('submit', (e) => this.handleSubmit(e));
        ...
        document.getElementById('manga-cover-file')?.addEventListener('change', (e) => this.handleCoverUpload(e));
        document.getElementById('remove-cover')?.addEventListener('click', () => this.removeCoverFile());
        document.getElementById('add-chapter-btn')?.addEventListener('click', () => this.addChapter());

        // User management events
        document.getElementById('manage-users-btn')?.addEventListener('click', () => this.showUsersSection());
        document.getElementById('back-to-mangas-btn')?.addEventListener('click', () => this.showMangasSection());
        ...
        document.getElementById('user-confirm-close')?.addEventListener('click', () => this.closeUserConfirmModal());
        document.getElementById('user-confirm-cancel')?.addEventListener('click', () => this.closeUserConfirmModal());
      ...
      createMangaCard(m) {
        ...
        const hasCover = m.cover && String(m.cover).trim() && m.cover !== 'undefined' && m.cover !== 'null';
        ...
        return `
          <div class="manga-card" data-id="${m.id}">
            <div class="manga-thumb">
              ${latest}
              <img src="${src}" alt="${m.title}" class="manga-cover" loading="lazy"
            ...
            <div class="manga-info">
              ...
              <div class="manga-meta">
                <span class="manga-chapters">${m.chapters ? m.chapters.length : 0} capítulos</span>
                <span class="manga-status ${statusClass}">${m.status}</span>
              </div>
              <div class="manga-actions">
                <button class="action-btn edit-btn" onclick="adminPanel.editManga(${m.id})">✏️ Editar</button>
                <button class="action-btn delete-btn" onclick="adminPanel.confirmDelete(${m.id})">🗑️ Excluir</button>
              </div>
            </div>
          </div>`;
      }
      ...
      updateStats() {
        ...
        const chapters = mangas.reduce((acc, m) => acc + (m.chapters ? m.chapters.length : 0), 0);
      ...
      openModal(manga = null) {
        ...
        document.getElementById('modal-title').textContent = this.isEditing ? 'Editar Mangá' : 'Adicionar Mangá';
        this.resetUploads();
        if (this.isEditing) this.populateForm(manga); else document.getElementById('manga-form')?.reset();
      ...
      populateForm(m) {
        ...
        document.getElementById('manga-status').value = m.status || 'Em andamento';
        document.getElementById('manga-latest').checked = !!(m.is_latest || m.isLatest);
      }
      ...
      async handleSubmit(e) {
        ...
        if (!this.isEditing && !this.selectedCoverFile) return this.showError('Capa é obrigatória');
        if (!this.isEditing && this.chapters.length === 0) return this.showError('Pelo menos um capítulo é obrigatório');
  
        for (const ch of this.chapters) {
          if (ch.files.length === 0) return this.showError(`Capítulo ${ch.number} precisa ter pelo menos uma página`);
        }
        ...
        const chaptersData = this.chapters.map(ch => ({ number: ch.number, filesCount: ch.files.length }));
        fd.append('chaptersData', JSON.stringify(chaptersData));
        this.chapters.forEach(ch => ch.files.forEach((file, i) => fd.append(`chapter_${ch.number}_page_${i}`, file)));
  
        try {
          const url = `${window.API_BASE_URL}/mangas`;
          const resp = await fetch(url, { method: this.isEditing ? 'PUT' : 'POST', headers: { Authorization: `Bearer ${token}` }, body: fd });
          if (resp.status === 401) {
          ...
          if (!resp.ok) {
            ...
            try {
              msg += ct.includes('json') ? `: ${(await resp.json()).message || resp.statusText}` : `: ${await resp.text()}`;
            } catch (_) { msg += `: ${resp.statusText}`; }
            throw new Error(msg);
          }
          await resp.json();
          this.closeModal();
          this.loadMangas();
          this.showSuccess(this.isEditing ? 'Mangá atualizado com sucesso!' : 'Mangá criado com sucesso!');
        } catch (err) {
      ...
      async deleteManga() {
        ...
        try {
          const token = authManager.getToken();
          const resp = await fetch(`${window.API_BASE_URL}/mangas/${this.currentManga.id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` } });
          if (!resp.ok) throw new Error((await resp.json()).message || 'Erro ao excluir mangá');
          this.closeConfirmModal();
          this.loadMangas();
      ...
      handleSearch(e) {
        ...
        this.renderMangas(this.mangas.filter(m => (m.title || '').toLowerCase().includes(q) || (m.description || '').toLowerCase().includes(q)));
      }
      ...
      handleFilter(e) {
        ...
        else if (f === 'ongoing') list = this.mangas.filter(m => (m.status || '').toLowerCase() === 'em andamento');
        else if (f === 'completed') list = this.mangas.filter(m => (m.status || '').toLowerCase() === 'finalizado');
      ...
      resetUploads() {
        ...
        const img = document.getElementById('cover-preview-img'); if (img) img.src = '';
        this.chapters = [];
        this.chapterCounter = 0;
        const cont = document.getElementById('chapters-container'); if (cont) cont.innerHTML = '';
      }
  
      handleCoverUpload(e) {
        const file = e.target.files[0];
        if (!file) return;
        if (!file.type.startsWith('image/')) { this.showError('Selecione apenas imagens.'); e.target.value=''; return; }
      ...
      addChapter() {
        ...
        cont.addEventListener('click', (e) => { if (e.target !== inp) inp.click(); });
      ...
      removeChapterFile(n, i) {
        ...
        if (this.chapters[idx].files.length) this.showChapterFilesPreview(n, this.chapters[idx].files);
        else {
          const item = document.querySelector(`[data-chapter-number="${n}"]`);
      ...
      showNotification(message, type) {
        ...
        Object.assign(n.style, { position:'fixed', top:'20px', right:'20px', padding:'1rem 1.5rem', borderRadius:'8px', color:'white', fontWeight:'600', zIndex:'3000', animation:'slideInRight 0.3s ease-out', backgroundColor: type==='success' ? '#10b981' : '#ef4444' });
      ...
      async showUsersSection() {
        ...
        document.getElementById('search-input').placeholder = 'Buscar usuários...';
      ...
      renderUsers() {
        ...
        grid.innerHTML = this.users.map(user => this.createUserCard(user)).join('');
      }
      ...
      async handleUserSubmit(e) {
        ...
        try {
          const token = authManager.getToken();
          const response = await fetch(`${window.API_BASE_URL}/users/${this.currentUser.id}/role`, {
            method: 'PUT',
            headers: {
      ...
      async deleteUser() {
        ...
        try {
          const token = authManager.getToken();
          const response = await fetch(`${window.API_BASE_URL}/users/${this.currentUser.id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`
            }
    ...
    if (!window.adminPanelInitialized) {
      document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('confirm-delete')?.addEventListener('click', () => window.adminPanel.deleteManga());
        document.getElementById('user-confirm-delete')?.addEventListener('click', () => window.adminPanel.deleteUser());
      });
    }
